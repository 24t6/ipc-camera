...
| 10 | **② `/recent.mp4?sec=N`**:把旁路裸流侧车**尾部**重封成小 MP4 放 `/tmp` ⇒ "刚录的这段"也能立刻看。待解决:mp4v2 跨线程串行化;重封期间录制队列会丢帧(与 A12 同代价) | ⬜ 设计已定, 未实现 |
| 11 | **③ 实时画面进浏览器**:硬件 JPEG 通道 + MJPEG(`/live.mjpg`)。⚠️ 通道必须**按需启停** —— 没人取流的编码通道会拖死整条流水线(B027) | ⬜ 未开始 |
| 12 | (可选)自研 C 客户端:**SDL2 + FFmpeg**(可选 ImGui);seek 直接映射成 HTTP `Range` | ⬜ 想讲"解复用→解码→上屏"时再做 |
| 13 | (可选)接入第三方 NVR(Frigate / ZoneMinder)证明标准可接入 | ⬜ |
