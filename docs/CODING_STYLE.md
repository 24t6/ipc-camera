...
### 9.2 ⚠️ 续行的对齐缩进**也算缩进层数**(2026-09-16 新增)

**`tools/check_style.py` 是按"行首空格 ÷ 4"算缩进的**,所以**把实参对齐到左括号**
那种写法(常见于参数多的调用)会被判成**深缩进**:

```c
    if (!MP4WriteSample(g.mp4, g.track, g_mp4buf, (uint32_t)(len + 4),
                        MP4_INVALID_DURATION, 0, is_key ? 1 : 0)) {
/*                      ↑ 24 空格 = 6 层 → 判违规            */
```

**这条今天(2026-09-16)一天之内踩了四次**
(`bsp_osd.c` / `svc_record_policy.c` / `svc_record.c` / `svc_media.c`),
而且四次都是"新写的代码" —— 说明它不是偶发, 是这个写法本身的必然结果。

**对策(按优先级)**:

| 对策 | 说明 |
|---|---|
| **① 抽 helper** | 把参数收进一个小函数(如 `write_mp4_sample(total, is_key)`), 调用处就短了。**首选** |
| ② 提前把中间值算成局部变量 | 如 `size_t pixels = W * H;` 再传 `pixels`, 让调用行变短 |
| ③ 续行只用 **8 空格** | 合规, 但不符合常规 C 习惯, 可读性差 |
| ④ 拆成多条语句 | 分步赋值再调用 |

> **不要**为了达标去删注释 —— 检查器已经明确把"总行数"(含注释)设为**软性**上限,
> 硬约束只针对**代码行**。删注释既不解决问题, 也违反 §4。
...
### 13.4 2026-09-20:按用户要求"复核最近写的代码有没有跑偏"

起因:用户问"你最近写的代码有没有符合规范?有没有跑偏?" ——
**这件事同样不能靠嘴回答**(§13 的起因一模一样)。

做法:① 跑三件套;② 用 `work/audit_new_funcs.py` 把**本轮动过的 15 个函数**逐个
量出"代码行 / 局部变量 / 缩进"(而不是只看"工具没报错");③ 用
`work/audit_style_manual.py` 查**机器查不到**的那几条(§6.1 动态分配 / §6.3 逐个
数组的字节数 / §7.1 线程命名 / §5.2 返回值清单), 再人工读一遍注释。

| 项 | 结果 |
|---|---|
| §9 行宽 / §2.2 前缀 / §3.1 guard / §3.2 文件头 / §4.1 Doxygen / §4.3 TODO | ✅ 三件套 **0 违规**(`check_spec` / `check_style` / `check_encoding`) |
| §9.1 函数规模(本轮 15 个函数) | ✅ 全部达标;**最紧的是 `handle_file()` 49 代码行**(因为加了 `hdrs` 参数), 离 50 只差 1 行 ⇒ 记为**观察项** |
| §6.1 运行期动态分配 | ✅ 本轮改过的文件里 `malloc/free` **0 处** |
| §6.3 栈上大数组 | ✅ 逐个算过:本轮新增的都是 `name[64]` / `hdrs[160]` / `path[256]` 量级(最大 256B);`HTTP_TEXT_BUF` 等 16KB 缓冲是**文件级静态结构体 `g` 的成员**, 不在栈上 |
| §7.1 线程命名 | ✅ 本轮没新增线程;`ipc_http` / `ipc_rec` 仍在 |
| **§6.2 缓冲四要素** | ❌→✅ 我新加的 `g_longfix` **只写了一句用途**, 缺"容量依据/内存区域/所有者/释放时机" ⇒ **已补** |
| **§2.3 避免魔法数字** | ❌→✅ `handle_recent()` 里写了 `char hdrs[160]` ⇒ 改成 `HTTP_RECENT_HDR_MAX`, 并把容量依据**算准**(47+15+63+2 = 127 ≤ 160;第一版注释里我写的是"23 字节名字"的**约数**, 也一并纠正) |
| **§4.1 `@note` 要写调用线程/阻塞行为** | ❌→✅ `newest_closed()` / `wait_closed()` 缺"调用线程 + 阻塞行为" ⇒ **已补**(后者会阻塞最多 5 秒, 必须写明) |
| **与既有约定的一致性** | ❌→✅ 我新写的 `wait_closed()` 用了 **`usleep`** —— 这是**全仓唯一一处**(别处一律 `nanosleep(&ts, NULL)`, 见 `svc_media.c` / `svc_osd.c`), 而且 `usleep` 已被 POSIX 移出标准 ⇒ 改成 `nanosleep` + 补 `#include <time.h>` |

> ⭐ **结论(诚实版)**:机器能查的那 8 条**一条没犯**;但"四要素 / 调用线程注释 /
> 魔法数字 / 跟着既有的睡法走"这 4 条**全犯了** —— 而这 4 条恰好都是
> §13.3 说的"钩子挡不住、仍靠 review"的那一类。
> **所以"工具绿了"只能证明机器那半边, 复核仍然要人(或按清单逐条)过一遍。**
>
> 复核后重跑:三件套 ✅ + 交叉编译**我们自己的代码零告警** ✅ + 宿主 `make test` 全绿 ✅ +
> 板上 `work/verify_recent.py` **17/17**(改了 `wait_closed` 的睡法 ⇒ 必须重跑它,
> 否则就是"改完没验")。
