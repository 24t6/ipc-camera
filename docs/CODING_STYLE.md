...
[ ] 跑过 `python tools/check_naming.py` 吗?(§2.2 分层前缀 / §3.2 文件头四要素 / §9 行宽 ≤100 列)
...
## 十三、2026-09-16:把"没人查的规矩"变成机器可查

起因:用户问"代码文件的编写应该按照我的规范来写吧?" ——
这件事**不能靠嘴回答**。查下来发现:§2.2 命名前缀、§3.2 文件头四要素、
§9 行宽**三条一直都是没人查的**,而"没人查的规矩等于没有"(规矩 8)。

于是补了 **`tools/check_naming.py`**:

```
python tools/check_naming.py        # 只报有问题的
python tools/check_naming.py --all  # 连每条违规的行号都列出来
```
...
| 条目 | 结果 |
|---|---|
| §2.2 分层前缀 | ✅ **0 处**(曾有 4 处, 在 `bsp/osd_render.h`;已改名 `bsp_osd_render.*`) |
| §3.2 文件头四要素 | ⚠️ **14 个 `.c` 缺**(存量问题, 不是新引入的);新增的 3 个已补 |
| §9 行宽 ≤ 100 列 | ✅ **0 处** |

> ⚠️ **§9 这一格的教训比结论更重要**:
> 我第一次是用 PowerShell 的 `.Length` 量的, 报出"**29 行超标**"(最长的 217 列)。
> **那是量错了** —— PowerShell 5.1 把 UTF-8 中文当 GBK 读, 长度虚高。
> 新工具改成**按 UTF-8 解码 + 按显示宽度算(CJK 算 2 列)** 之后, 结论是**全都合格**。
> **差一点就去"修"29 行本来没坏的东西。**
> → 这与 **B020**(量口径错了三次) / **B026**(验证脚本自己骗人)是同一类:
> **测量口径错了, 结论一定错, 而且会带着你去改正确的东西。**

**待办**:给存量那 14 个 `.c` 补文件头四要素(纯体力活, 一次提交做完即可)。
