 ...
 *   不再是"看着像正常文件、其实播不了"。
 *
 *   ⚠️ `.tmp` 的**代价**是"最近这一段看不到"(缺口 = 段长)。2026-09-20 加的
 *   `svc_record_request_rotate()` 就是为此:**回放服务可以请录制线程提前收尾当前段**
 *   (在下一个 IDR 处, 与"到点切段"走同一条路), 一秒钟后它就变成正常 `.mp4` ——
 *   于是"看最新"的缺口从"段长"降到 1 秒左右(见 `STATUS.md` 的 A20)。
 *
 ...
 *       "刚录的画面"马上变成可播的 `.mp4`。但如果有人连点几下, 每点一次都切一刀,
 *       就会切出一串**几秒钟的碎段**(文件数暴涨、每段一个 moov, 还浪费环形容量)。
 *       所以:**段龄不足这个秒数的段不切**, 调用方改用"已经收尾的最新那段"顶上。
 */
#define SVC_RECORD_ROTATE_MIN_SEC 20

/**
 * @brief 请求**尽快收尾当前正在写的那一段**(回放服务的 `/recent.mp4` 用)。
 *
 * @param[out] name    回填:当前正在录的那一段的名字(形如 `<stamp>.mp4`);可 NULL
 * @param[in]  cap     `name` 的容量
 * @param[out] age_sec 回填:这一段已经录了多少秒(-1 = 不知道);可 NULL
 * @return **0** = 已经请求切段(录制线程会在**下一个 IDR** 处收尾, 通常 ≤1 秒);
 *         **1** = 段太新(< `SVC_RECORD_ROTATE_MIN_SEC`), **不切** —— 请用已有的最新段;
 *         **-1** = 现在没有在录(或参数非法)
 *
 * @note ★ **边界仍然对齐 IDR**:请求只是把 `want_close` 那条路提前触发,
 *       录制线程仍是"遇到下一个关键帧才关旧段、并用这一帧开新段" ⇒
 *       切出来的段**一帧不丢、可独立解码**(与"到点切段"完全同一条路径)。
 * @note ⚠️ **跨线程**:本函数从 HTTP 线程调用, 内部用互斥锁置一个请求标志;
 *       真正动文件的是**录制线程**。mp4v2 的句柄自始至终只有录制线程碰。
 * @note ⚠️ 【简化上限】只有"一个待处理的请求"(不是队列):连点多次 = 切一次。
 *       天花板:极高频调用下, 段长会被压到 `SVC_RECORD_ROTATE_MIN_SEC` 附近。
 *       升级路径:若要做"每客户端一个独立片段", 那时得改成"把请求交给独立的重封线程",
 *       并按 B027 的教训处理"没有消费者的通道/队列"。
 */
