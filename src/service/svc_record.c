 ...
 *             这个文件里其实是**六件事**(按分节横幅自上而下):
 *               ① 文件名/路径小工具  ② 锁定清单 + 环形覆盖  ③ 旁路裸流侧车 + 残留清理
 *               ④ 分段开关 + 写帧(mp4v2)  ⑤ 掉电残留重封(把 `.h264.tmp` 救回来)
 *               ⑥ 对外接口(生命周期 + 回放服务要用的四个查询/控制)
 *             六件事共用一个文件级 `g`(录制线程独占)⇒ 它们**不是**可以随手拆开的关系;
 *             要拆先读 `CODING_STYLE.md` §9.1(不以行数为由拆文件)。
 ...
 *
 * @note ⚠️ 【简化上限】**清单的行格式在这里是"第二次实现"** ——
 *       真相源在 `svc_record_policy.c` 的 `line_is_name()` / `lock_has()`
 *       (纯函数、有 PC 单测):**跳空行、跳 `#` 注释、裁掉行尾 CR/空格/制表符**。
 *       这里为了"只把文件顺序读一遍、读进固定缓存"而自己写了同一套裁剪规则。
 *       · 天花板: 两处规则**必须一直保持一致**。一旦一边改了(例如允许行尾写注释),
 *         就会出现"回放服务锁上了、但环形覆盖不认这个锁 ⇒ 把用户锁的段删掉"
 *         —— B043 已经演示过同型事故(锁状态在两条路径上不一致)。
 *       · 为什么不现在就抽掉: 收益是 ~25 行, 代价是动**锁定路径**(这条路径出 bug 的
 *         后果是删掉用户的录像), 且要重跑 A15/A19 两条验收。按"只保留 K 个/不做
 *         投机性抽象"的口径(AGENTS.md §7.0.1 台阶 1、`CODING_STYLE` §9.1),
 *         现在**保持原样**更划算 —— 但**登记在此**, 不允许悄悄漂移。
 *       · 升级路径: 把"逐行取名字"做成策略层的一个纯函数
 *         (如 `svc_record_policy_lock_names(buf, used, cb, user)`), 服务层只负责
 *         `fread` 文件; 那时这份重复自然消失, 而且新纯函数能在 PC 上单测。
 */
...
/* ─────────── 回放服务要用的四个接口(阶段 2) ───────────
 *   ① svc_record_list()           列最新一页(分页游标 before)
 *   ② svc_record_get_status()     正在写的那一段(名字/字节/帧数/起始时间)
 *   ③ svc_record_set_lock()       锁定 / 解锁(改 `.locked` 清单)
 *   ④ svc_record_request_rotate() 请录制线程在下一个 IDR 处收尾当前段("/recent.mp4")
 * ⚠️ 章节目录里写"三个"是 2026-09-20 之前的旧状态 —— 加了 ④ 之后必须同步(§4.3)。
 */
