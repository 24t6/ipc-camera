...
#define BSP_OSD_RENDER_MAX_CHARS 24

/** 一个字形在字模里的尺寸(像素) */
#define BSP_OSD_RENDER_GLYPH_W 8
#define BSP_OSD_RENDER_GLYPH_H 16

/**
 * 文字像素: ARGB1555, **alpha 位 = 1** → 按 `u32FgAlpha` 混叠。
 * 0xFFFF = 不透明白。
 */
#define BSP_OSD_RENDER_PIXEL_FG 0xFFFFu

/**
 * 背景像素: ARGB1555, **alpha 位 = 0** → 按 `u32BgAlpha` 混叠。
 * 0x0000 配上 `u32BgAlpha = 0` 就是**全透明**, 视频从水印底下透出来。
 */
#define BSP_OSD_RENDER_PIXEL_BG 0x0000u
 ...
 * @note 纯函数, 不分配内存。**背景会被完整填成 BSP_OSD_RENDER_PIXEL_BG**,
 *       所以调用方不需要预先清零。
 */
